...
/// <summary>
/// Anonymous-user upload via a box share link, exercised against real S3 providers.
/// Counterpart to <c>s3_box_share_link_tests</c>: where that one validates the
...
public class s3_box_link_upload_tests : TestFixture
{
    private readonly S3LiveStoragesFixture _liveFixture;
    private AppSignedInUser AppOwner { get; }

    public s3_box_link_upload_tests(
        ...
        StorageEncryptionType encryptionType)
    {
        ...
        initiate.MultiStepChunkUploads.Should().HaveCount(1,
            "a 12 MB file on S3 storage must resolve to MultiStepChunkUpload regardless of None/Managed encryption");
