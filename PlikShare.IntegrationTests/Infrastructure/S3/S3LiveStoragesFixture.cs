...
/// <summary>
/// Collection-scoped fixture that owns one shared <see cref="S3LiveSetup"/> per
...
/// </summary>
public sealed class S3LiveStoragesFixture : IAsyncLifetime
{
    private readonly List<S3LiveSetup> _setups = [];
    ...
    public async Task<S3LiveSetup> GetOrCreate(
        ...
        TestFixture.AppSignedInUser user)
    {
        ...
        try
        {
            ...
            // password (inside CreateS3Storage → SetupUserEncryptionPassword). Reset wipes
            ...
            var storage = await testFixture.CreateS3Storage(
                effectiveUser,
                provider, 
                encryption);
            ...
            Log.Information("[S3LiveStoragesFixture] Created setup for {Provider}/{Encryption}: storage={StorageExtId}, workspace={WorkspaceExtId}, bucket={BucketName}",
    ...
    public async Task DisposeAsync()
    {
        ...
        foreach (var setup in _setups)
        {
            ...
            {
                Log.Error(e, "[S3LiveStoragesFixture] Error cleaning up setup {Provider}/{Encryption}",
                    setup.Provider, setup.Encryption);
            }
        }
    }

    private async Task CleanupSetup(S3LiveSetup setup)
    {
        ...
        {
            Log.Warning("[S3LiveStoragesFixture] delete-bucket queue job for storage#{StorageId} did not complete in time — falling back to S3HardPurge for bucket '{BucketName}' on {Provider}.",
