...
@Component({
    selector: 'app-edit-azure-blob-storage',
    imports: [
        FormsModule,
        MatFormFieldModule,
        MatInputModule,
    ...
    templateUrl: './edit-azure-blob-storage.component.html',
    styleUrl: './edit-azure-blob-storage.component.scss',
    encapsulation: ViewEncapsulation.None
})
export class EditAzureBlobStorageComponent {
    isLoading = signal(false);
    couldNotConnect = signal(false);

    authType = new FormControl<AzureBlobAuthType>('shared-key', Validators.required);
    encryptionType = new FormControl<AppStorageEncryptionType>('none', Validators.required);
    ...
    constructor(
        ...
        public dialogRef: MatDialogRef<EditAzureBlobStorageComponent>,
        ...
        this.authType.setValue(data.authType ?? 'shared-key');
        this.encryptionType.setValue(data.encryptionType ?? 'none');
        this.accountName.setValue(data.accountName ?? '');
        this.serviceUrl.setValue(data.serviceUrl ?? '');
        this.sasToken.setValue(data.sasToken ?? '');
        this.managedIdentityClientId.setValue(data.managedIdentityClientId ?? '');
    ...
    async onSave() {
        ...
        try {
            ...
            await this._storagesApi.updateAzureBlobStorageDetails(this.data.storageExternalId, {
                ...
                encryptionType: this.encryptionType.value!,
