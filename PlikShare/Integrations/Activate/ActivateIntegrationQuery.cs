...
public class ActivateIntegrationQuery(DbWriteQueue dbWriteQueue)
{
        ...
        IntegrationExtId externalId)
    {
        ...
        if (result.IsEmpty)
        {
            Log.Warning("Could not activate Integration '{IntegrationExternalId}' because it was not found.",
                externalId);
        ...
        Log.Information("Integration {IntegrationType} '{IntegrationExternalId}' was activated",
            result.Value.Type,
            externalId);
