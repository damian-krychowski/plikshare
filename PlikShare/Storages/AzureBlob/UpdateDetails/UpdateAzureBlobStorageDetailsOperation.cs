...
namespace PlikShare.Storages.AzureBlob.UpdateDetails;

public class UpdateAzureBlobStorageDetailsOperation(
    ...
    PreSignedUrlsService preSignedUrlsService)
{
    public async Task<ResultCode> Execute(
        ...
        AzureBlobDetailsEntity newDetails,
        StorageEncryptionType encryptionType,
        CancellationToken cancellationToken = default)
    {
        var newClientResult = await AzureBlobStorageClient.BuildAndTestConnection(
            details: newDetails,
            cancellationToken: cancellationToken);

        if (newClientResult.Code == AzureBlobStorageClient.AzureBlobResultCode.InvalidUrl)
            return ResultCode.InvalidUrl;

        if (newClientResult.Code == AzureBlobStorageClient.AzureBlobResultCode.CouldNotConnect)
        ...
        var result = await updateStorageDetailsQuery.Execute(
            ...
            encryptionType: encryptionType,
            encryptionDetailsJson: encryptionDetails is null
                ? null
                : Json.Serialize(item: encryptionDetails),
            cancellationToken: cancellationToken);
    ...
    private void RegisterClient(
        ...
        AzureBlobStorageClient.AzureBlobResult newClientResult)
    {
        ...
        storageClientStore.RegisterClient(new AzureBlobStorageClient(
            ...
            clock: clock,
            blobServiceClient: newClientResult.Client!,
