    ...
    IClock clock)
{
    public ArchiveAuditLogsResponseDto Execute(string? olderThanDate)
    {
        ...
        var sql = olderThanDate is not null
            ? "SELECT al_external_id, al_created_at, al_correlation_id, al_actor_identity_type, al_actor_identity, al_actor_email, al_actor_ip, al_event_category, al_event_type, al_event_severity, al_resource_type, al_resource_external_id, al_resource_name, al_workspace_external_id, al_details FROM al_audit_logs WHERE al_created_at < $cutoff ORDER BY al_id ASC"
            : "SELECT al_external_id, al_created_at, al_correlation_id, al_actor_identity_type, al_actor_identity, al_actor_email, al_actor_ip, al_event_category, al_event_type, al_event_severity, al_resource_type, al_resource_external_id, al_resource_name, al_workspace_external_id, al_details FROM al_audit_logs ORDER BY al_id ASC";
        ...
        var items = connection
            .Cmd(
                ...
                readRowFunc: reader => new AuditLogItemDto
                {
                    ...
                    ResourceType = reader.GetStringOrNull(10),
                    ResourceExternalId = reader.GetStringOrNull(11),
                    ResourceName = reader.GetStringOrNull(12),
                    WorkspaceExternalId = reader.GetStringOrNull(13),
                    Details = reader.GetStringOrNull(14)
                });
