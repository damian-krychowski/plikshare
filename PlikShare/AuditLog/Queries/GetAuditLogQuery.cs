            ...
            .Cmd(
                sql: """
                    ...
                    WHERE ($cursor IS NULL OR al_id < $cursor)
                      ...
                      AND ($actorIdentities IS NULL
                          OR al_actor_email IN (SELECT value FROM json_each($actorIdentities))
                          OR al_actor_identity IN (SELECT value FROM json_each($actorIdentities)))
