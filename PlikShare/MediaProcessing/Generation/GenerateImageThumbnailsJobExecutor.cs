...
public class GenerateImageThumbnailsJobExecutor(
...
{
    private static readonly Serilog.ILogger Logger = Log.ForContext<GenerateImageThumbnailsJobExecutor>();
    ...
    public static string StaticJobType => GenerateImageThumbnailsJobType.Value;
    ...
    {
        var definition = Json.Deserialize<GenerateImageThumbnailsJobDefinition>(definitionJson)
    ...
    private static void FindExistingThumbnailsToDelete(
        ...
        Dictionary<int, List<ExistingThumbnail>> existingThumbnails,
    ...
    private async Task<OneFileResult> ProcessOneFile(
        ...
        GenerateImageThumbnailsJobDefinition definition,
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            foreach (var result in variantResults!)
            {
                ...
                {
                    Logger.Error(
                        "Failed to generate {Variant} thumbnail for File#{FileId}: {Error}",
                        result.Variant,
                        parentFile.Id,
                        result.Error);
                    ...
                    using (encryptionSeed)
                    {
                        ...
                        readyThumbnailFiles.Add(new BulkInsertFileEntity
                        {
                            ...
                            FileName = encryptionSeed
                                .DeriveNew()
                                .EncodeMetadata($"thumb-{result.Variant.ToString().ToLowerInvariant()}"),
                ...
                catch (Exception ex)
                {
                    Logger.Error(
                        ex,
                        "Failed to upload {Variant} thumbnail for File#{FileId}.",
                        result.Variant,
                        parentFile.Id);
        ...
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            Logger.Error(
                ex,
                "Storage/ffmpeg failed for File#{FileId}.",
                parentFile.Id);
        ...
        GenerateImageThumbnailsJobDefinition definition)
    {
        ...
        var encryptionSeedKey = GenerateImageThumbnailsJobDefinition.GetVariantEncryptionSeedsKey(
            fileId: parentFileId,
            variant: variant);

        var (seed, error) = ResolveEncryptionSeed(
            ephemeralSeed: definition.EncryptionSeeds?[encryptionSeedKey],
            workspace: workspace,
        ...
        int parentFileId)
    {
        ...
        {
            Logger.Error(
                "Full-encryption file#{FileId} in workspace#{WorkspaceId} has no ephemeral encryption seed in the job definition.",
                parentFileId,
                workspace.Id);
        ...
        {
            Logger.Warning(
                "Ephemeral encryption key expired for full-encryption file#{FileId} in workspace#{WorkspaceId} — the job is older than the key TTL or the service was restarted. Skipping thumbnail generation.",
                parentFileId,
                workspace.Id);
        ...
        Logger.Error(
            "Ephemeral encryption key could not be decoded ({Status}) for full-encryption file#{FileId} in workspace#{WorkspaceId}.",
            status,
            parentFileId,
            workspace.Id);
    ...
    private static ThumbnailGenerationResult.FileResult BuildFailedResult(
        ...
        string error)
    {
        var failed = new List<ThumbnailGenerationResult.FailedVariant>(
            variants.Length);
        ...
        return BuildFileResult(fileExternalId, [], failed);
    }
    ...
    private (List<FileEntity> ParentFiles, Dictionary<int, List<ExistingThumbnail>> ExistingThumbnails) GetRelatedFiles(
        GenerateImageThumbnailsJobDefinition definition,
        WorkspaceContext workspace)
    {
        ...
        int[] fileIds =
        [
            ..definition.ImageFileIds,
            ..definition.VideoFileIds
        ];
        ...
        var parentFiles = connection
            .Cmd(
                ...
                readRowFunc: (reader) =>
                {
                    ...
                    var encryptionSeedKey = GenerateImageThumbnailsJobDefinition.GetFileEncryptionSeedsKey(
                        fileId: id);
                    ...
                    return new FileEntity
                    {
                        ...
                        EncryptionSeed = definition.EncryptionSeeds?[encryptionSeedKey],
                        IsVideo = definition.VideoFileIds.Contains(id)
                    };
                })
            .WithJsonParameter("$ids", fileIds)
    ...
    //TODO: does not work for full encryption workspaces
    private static Dictionary<int, List<ExistingThumbnail>> GetExistingThumbnails(
        WorkspaceContext workspace, 
        int[] parentIds, 
        ...
        return connection
            .AggregateRows(
                sql: """
                     ...
                     """,
                seed: new Dictionary<int, List<ExistingThumbnail>>(),
                aggregateRowFunc: (acc, reader) =>
                {
                    ...
                    var parentFileId = reader.GetInt32(3);

                    if (!acc.TryGetValue(parentFileId, out var thumbnails))
                    {
                        thumbnails = [];
                        acc[parentFileId] = thumbnails;
                    }
            ...
            .WithJsonParameter("$fileIds", parentIds)
            .WithParameter("$workspaceId", workspace.Id)
            .Execute();
    }
