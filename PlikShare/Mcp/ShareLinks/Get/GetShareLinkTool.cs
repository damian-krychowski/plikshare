...
public class GetShareLinkTool
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parameters = new GetShareLinkParams
        {
            WorkspaceExternalId = workspaceExternalId,
            ShareLinkExternalId = shareLinkExternalId
        };

        var definition = AgentToolCatalog.TryGet(AgentToolNames.GetShareLink)!;

        var workspaceOverride = workspaceToolOverrideReader.TryGet(
            agentId: agent.Id,
            workspaceId: workspace.Id,
            toolName: AgentToolNames.GetShareLink);
        ...
        if (!effective.IsUsable)
            throw new McpException("The get_share_link tool is not enabled for this workspace.");
        ...
        var result = await getShareLinkOperation.Execute(
            httpContext,
            parameters,
            cancellationToken);
