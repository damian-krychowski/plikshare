...
public class GetBoxTool
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parameters = new GetBoxParams
        {
            WorkspaceExternalId = workspaceExternalId,
            BoxExternalId = boxExternalId
        };

        var definition = AgentToolCatalog.TryGet(AgentToolNames.GetBox)!;

        var workspaceOverride = workspaceToolOverrideReader.TryGet(
            agentId: agent.Id,
            workspaceId: workspace.Id,
            toolName: AgentToolNames.GetBox);
        ...
        if (!effective.IsUsable)
            throw new McpException("The get_box tool is not enabled for this workspace.");
        ...
        var result = await getBoxOperation.Execute(
            httpContext,
            parameters,
            cancellationToken);
