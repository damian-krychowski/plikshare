...
[McpServerToolType]
public class ListWorkspaceBoxesTool
{
    [McpServerTool(Name = AgentToolNames.ListWorkspaceBoxes)]
    [Description("Lists the boxes of a workspace the agent can access, with their external ids, names, " +
                 "enabled state and the folder each box exposes. If this tool requires approval the call " +
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parameters = new ListWorkspaceBoxesParams
        {
            WorkspaceExternalId = workspaceExternalId
        };

        var definition = AgentToolCatalog.TryGet(AgentToolNames.ListWorkspaceBoxes)!;

        var workspaceOverride = workspaceToolOverrideReader.TryGet(
            agentId: agent.Id,
            workspaceId: workspace.Id,
            toolName: AgentToolNames.ListWorkspaceBoxes);
        ...
        if (!effective.IsUsable)
            throw new McpException("The list_workspace_boxes tool is not enabled for this workspace.");
        ...
        var result = await listWorkspaceBoxesOperation.Execute(
            httpContext,
            parameters,
            cancellationToken);
