...
public class ListBoxLinksTool
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parameters = new ListBoxLinksParams
        {
            WorkspaceExternalId = workspaceExternalId,
            BoxExternalId = boxExternalId
        };

        var definition = AgentToolCatalog.TryGet(AgentToolNames.ListBoxLinks)!;

        var workspaceOverride = workspaceToolOverrideReader.TryGet(
            agentId: agent.Id,
            workspaceId: workspace.Id,
            toolName: AgentToolNames.ListBoxLinks);
        ...
        if (!effective.IsUsable)
            throw new McpException("The list_box_links tool is not enabled for this workspace.");
        ...
        var result = await listBoxLinksOperation.Execute(
            httpContext,
            parameters,
            cancellationToken);
