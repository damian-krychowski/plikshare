...
public class MoveItemsTool
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parameters = new MoveItemsParams
        {
        ...
        var definition = AgentToolCatalog.TryGet(AgentToolNames.MoveItems)!;

        var workspaceOverride = workspaceToolOverrideReader.TryGet(
            agentId: agent.Id,
            workspaceId: workspace.Id,
            toolName: AgentToolNames.MoveItems);
        ...
        if (!effective.IsUsable)
            throw new McpException("The move_items tool is not enabled for this workspace.");
        ...
        var result = await moveItemsOperation.Execute(
            httpContext,
            parameters,
            cancellationToken);
