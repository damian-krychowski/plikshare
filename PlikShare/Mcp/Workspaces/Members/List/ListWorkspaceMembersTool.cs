...
[McpServerToolType]
public class ListWorkspaceMembersTool
{
    [McpServerTool(Name = AgentToolNames.ListWorkspaceMembers)]
    [Description("Lists the members of a workspace the agent can access, with their external ids, emails, " +
                 "who invited them, whether they have accepted the invitation and whether they may share " +
                 "the workspace further. If this tool requires approval the call returns status " +
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parameters = new ListWorkspaceMembersParams
        {
            WorkspaceExternalId = workspaceExternalId
        };

        var definition = AgentToolCatalog.TryGet(AgentToolNames.ListWorkspaceMembers)!;

        var workspaceOverride = workspaceToolOverrideReader.TryGet(
            agentId: agent.Id,
            workspaceId: workspace.Id,
            toolName: AgentToolNames.ListWorkspaceMembers);
        ...
        if (!effective.IsUsable)
            throw new McpException("The list_workspace_members tool is not enabled for this workspace.");
        ...
        var result = await listWorkspaceMembersOperation.Execute(
            httpContext,
            parameters,
            cancellationToken);
