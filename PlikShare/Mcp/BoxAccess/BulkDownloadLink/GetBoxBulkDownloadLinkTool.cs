...
[McpServerToolType]
public class GetBoxBulkDownloadLinkTool
{
    [McpServerTool(Name = AgentToolNames.GetBoxBulkDownloadLink)]
    [Description("Creates a short-lived link that downloads several files and/or folders from a box as one " +
                 "ZIP archive. All ids must live inside the box. Pass expiresInMinutes to control the link's " +
    ...
    public static async Task<AgentToolResponse> Execute(
        ...
        [Description("External ids of the files to include. Pass an empty array if none.")]
        string[] fileExternalIds,
        [Description("External ids of the folders to include. Pass an empty array if none.")]
        string[] folderExternalIds,
        [Description("Optional external ids of files to exclude from the included folders.")]
        string[]? excludedFileExternalIds = null,
        [Description("Optional external ids of folders to exclude from the included folders.")]
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var parameters = new GetBoxBulkDownloadLinkParams
        {
        ...
        var definition = AgentToolCatalog.TryGet(AgentToolNames.GetBoxBulkDownloadLink)!;

        var boxOverride = boxToolOverrideReader.TryGet(
            agentId: agent.Id,
            boxId: box.Id,
            toolName: AgentToolNames.GetBoxBulkDownloadLink);
        ...
        if (!effective.IsUsable)
            throw new McpException("The get_box_bulk_download_link tool is not enabled for this box.");
        ...
        var result = await getBoxBulkDownloadLinkOperation.Execute(
            httpContext,
            parameters,
            cancellationToken);
