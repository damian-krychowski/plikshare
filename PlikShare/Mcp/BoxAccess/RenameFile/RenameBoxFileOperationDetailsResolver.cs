...
/// <summary>
/// Resolves a rename_box_file operation's stored ids into the box name, the file's current name (with
/// extension), its parent folder and path, and the requested new name, so a human reviewing the
/// approval sees exactly what gets renamed and to what — including the extension that is kept.
/// </summary>
public class RenameBoxFileOperationDetailsResolver(
    BoxApprovalNameResolver boxNameResolver,
    AuditLogService auditLogService,
    PlikShareDb plikShareDb)
{
    public RenameBoxFileOperationDetails Resolve(AgentOperation operation)
    {
        ...
        return new RenameBoxFileOperationDetails
        {
