using System.ComponentModel;
using ModelContextProtocol;
using ModelContextProtocol.Server;
using PlikShare.Agents.BoxAccess;
using PlikShare.Agents.Middleware;
using PlikShare.Agents.Operations;
using PlikShare.Agents.Tools;
using PlikShare.Boxes.Cache;
using PlikShare.Boxes.Id;
using PlikShare.Core.Clock;
using PlikShare.Core.Utils;
using PlikShare.Folders.Id;

namespace PlikShare.Mcp.BoxAccess.CreateFolder;

[McpServerToolType]
public class CreateBoxFolderTool
{
    [McpServerTool(Name = AgentToolNames.CreateBoxFolder)]
    [Description("Creates a new folder inside a box the agent was granted access to. Pass " +
                 "parentFolderExternalId to create a subfolder, or leave it empty to create the folder at the " +
                 "box's root. The parent must live inside the box. If this tool requires approval the call " +
                 "returns status 'waits_for_approval' with an approvalRequestId - poll check_approvals and, " +
                 "once approved, call execute_operation to run it.")]
    public static async Task<AgentToolResponse> Execute(
        IHttpContextAccessor httpContextAccessor,
        BoxCache boxCache,
        AgentBoxAccessCache boxAccessCache,
        AgentBoxToolOverrideReader boxToolOverrideReader,
        CreateBoxFolderAgentOperation createBoxFolderOperation,
        AgentOperationLedger operationLedger,
        AgentOperationsOptions operationsOptions,
        IClock clock,
        [Description("External id of the box.")]
        string boxExternalId,
        [Description("Name of the new folder.")]
        string name,
        [Description("Optional external id of the parent folder inside the box. Leave empty for the box root.")]
        string? parentFolderExternalId = null,
        CancellationToken cancellationToken = default)
    {
        var httpContext = httpContextAccessor.HttpContext!;
        var agent = await httpContext.GetAgentContext();

        var boxAccess = await httpContext.GetAgentBoxAccess(
            agent,
            boxCache,
            boxAccessCache,
            BoxExtId.Parse(boxExternalId),
            cancellationToken);

        var box = boxAccess.Box;

        var parameters = new CreateBoxFolderParams
        {
            BoxExternalId = boxExternalId,
            FolderExternalId = FolderExtId.NewId().Value,
            Name = name,
            ParentFolderExternalId = string.IsNullOrWhiteSpace(parentFolderExternalId)
                ? null
                : parentFolderExternalId
        };

        var definition = AgentToolCatalog.TryGet(AgentToolNames.CreateBoxFolder)!;

        var boxOverride = boxToolOverrideReader.TryGet(
            agentId: agent.Id,
            boxId: box.Id,
            toolName: AgentToolNames.CreateBoxFolder);

        var effective = AgentToolCatalog.Resolve(
            agent,
            definition,
            workspaceOverride: null,
            boxOverride: boxOverride);

        if (!effective.IsUsable)
            throw new McpException("The create_box_folder tool is not enabled for this box.");

        if (effective.RequiresApproval)
        {
            var expiresAt = clock.UtcNow.AddHours(
                operationsOptions.ApprovalWindowHours);

            var operationId = await operationLedger.CreatePending(
                agentId: agent.Id,
                workspaceId: box.Workspace.Id,
                toolName: AgentToolNames.CreateBoxFolder,
                paramsJson: Json.Serialize(parameters),
                expiresAt: expiresAt,
                cancellationToken: cancellationToken);

            return AgentToolResponse.WaitsForApproval(
                approvalRequestId: operationId.Value,
                expiresAt: expiresAt);
        }

        var result = await createBoxFolderOperation.Execute(
            httpContext,
            parameters,
            cancellationToken);

        return AgentToolResponse.Executed(
            result);
    }
}
